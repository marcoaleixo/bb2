...
def setup_args():
    ...
    parser_grp.add_argument('--port', default=35496, type=int, help='Port used by the web socket (run.py)')
    parser_grp.add_argument('--host', default='0.0.0.0', type=str, help='Host from which allow requests, use 0.0.0.0 to allow all IPs',)
    parser_grp.add_argument('--serving_port', default=8080, type=int, help='Port used to configure the server',)
...
if __name__ == "__main__":
    ...
    while error:
        ...
        try:
            ...
            ws = websocket.WebSocketApp("ws://api_core:{}/websocket".format(port), on_message=on_message, on_error=on_error, on_close=on_close)
            ws.on_open = on_open
            setup_interactive(ws)
            ws.run_forever()
        except Exception as error:
            print("Retrying the connection with API...")
            time.sleep(1)
